...
var validIPAddress = function(IP) {
    ...
    if(IP.includes('.')){
        ...
        for(var i = 0; i < IPV4.length; i++){
            if( !(numChecker(IPV4[i])) ){
                return 'Neither';
            }

            else if( IPV4[i].match(/[a-zA-Z]/i) ) {
                   return 'Neither';
                }

            else if( parseInt(IPV4[i]) > 255 || parseInt(IPV4[i]) < 0 ){
                return 'Neither';
            }

            else if( (IPV4[i].length > 1 && IPV4[i][0] === '0') || IPV4[i] === '') {
                return 'Neither';
            }
        }
        return 'IPv4';
        ...
        for( var x = 0; x < IPV6.length; x++ ){
            var temp = IPV6[x];
            if( IPV6[x].length > 4 || IPV6[x].length < 1 ){
                return 'Neither';
            } else if(IPV6[x].match(/[^a-fA-F0-9]/) !== null) {
                return 'Neither';
          }
